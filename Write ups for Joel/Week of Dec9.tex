...
\item Jump to that particle and record the resistance of the jump as $R_{\text{jump}} = R_t e^{d/\lambda}$ where $d$ is now the actual distance of the jump.
...
\section{effect of the residual force on the current history}

The strength of the residual force is taken to be given by the parameter $\omega$.

\begin{figure}[H]
--\includegraphics[width=0.3\textwidth]{last_frame_w-0-100elec.png}
\includegraphics[width=0.3\textwidth]{20e-frame_499_delay-0.01s.png}
\includegraphics[width=0.3\textwidth]{last_frame_w-10.png}
\caption{From left to right we show $\omega = 0, 1, 10$.}
\end{figure}

We see that this range of $\omega$ has essentially no effect on the current history. In addition, the residual forces are not physical so I would move to completely remove them from the simulation.
